...
def print_header():
    console.print(Panel.fit(
        ...
        "[dim]قابلیت جست وجو،تکی،گروهی،منطق های متنوع[/dim]",
        border_style="green",
        padding=(0, 2),
...
def find_database():
    ...
    table = Table(title="انتخاب دیتابیس", box=box.SIMPLE)
...
def create_backup(db_path):
    ...
    try:
        ...
        console.print(f"[bold red]❌ خطا در بکاپ : {e}[/bold red]")
        sys.exit(1)
...
def select_client_interactive(clients, real_usage):
    while True:
        console.print(Panel("برای نمایش همه اینتر بزنید یا بخشی از ایمیل را بنویسید", title="جستجوی کاربر"))
        ...
        table.add_column("Usage (GB)", style="green")
        ...
        for local_idx, (real_idx, c) in enumerate(filtered):
            ...
            usage_str = f"{bytes_to_gb(u_val)} / {'Inf' if t_val<=0 else bytes_to_gb(t_val)}"
        ...
        console.print(f"\n[dim]نمایش {len(filtered)} نتیجه.[/dim]")
        choice_str = Prompt.ask("شماره ردیف را انتخاب کنید (0 برای بازگشت)")
        
        try:
            choice = int(choice_str)
            if choice == 0: return None
            if 1 <= choice <= len(filtered):
                selected_real_index = filtered[choice-1][0]
                return clients[selected_real_index]
            else:
                console.print("[red]شماره نامعتبر.[/red]")
        except:
            console.print("[red]ورودی نامعتبر.[/red]")
...
def process_single_user_menu(client, usage_map):
    ...
    status_msg = "فعال" if enable else "غیرفعال"
    if expiry > 0 and expiry < (time.time()*1000): status_msg += " (منقضی)"
    if total > 0 and used >= total: status_msg += " (حجم تمام)"

    console.print(Panel(
        f"📧 [bold]{email}[/bold]\n"
        f"📊 مصرف : {bytes_to_gb(used)} / {'نامحدود' if total<=0 else bytes_to_gb(total)} GB\n"
        f"📅 انقضا : {timestamp_to_date(expiry)}\n"
        f"وضعیت : {status_msg}",
        title="مدیریت کاربر", style="blue"
    ))

    console.print("[0] بازگشت")
    console.print("[1] تمدید زمان (افزودن به انتهای زمان فعلی)")
    console.print("[2] احیا کردن (افزودن زمان از لحظه حال)")
    console.print("[3] افزایش حجم (افزودن به کل)")
    console.print("[4] ریست کردن حجم مصرفی (Reset Usage)")
    console.print("[5] تنظیم دستی (Set Values)")
    ...
    if action == 1:
        ...
        if days > 0:
            ...
            if expiry > current_time: base = expiry
            updates['expiryTime'] = base + (days * 86400000)

    elif action == 2:
        days = IntPrompt.ask("تعداد روز اعتبار (از همین لحظه)")
        if days > 0:
            updates['expiryTime'] = int(time.time() * 1000) + (days * 86400000)

    elif action == 3:
        gb = FloatPrompt.ask("مقدار گیگابایت افزودنی")
        if gb > 0:
            updates['totalGB'] = total + int(gb * 1073741824)
            updates['total'] = updates['totalGB']

    elif action == 4:
        if Confirm.ask("آیا مصرف کاربر صفر شود؟"):
            updates['RESET_USAGE'] = True
            
    elif action == 5:
        new_days = IntPrompt.ask("اعتبار به روز (0=نامحدود)", default=-1)
        if new_days >= 0:
            updates['expiryTime'] = 0 if new_days==0 else int(time.time()*1000) + (new_days*86400000)
        
        new_gb = FloatPrompt.ask("حجم کل GB (0=نامحدود)", default=-1.0)
        if new_gb >= 0:
...
def main():
    ...
    try:
        ...
        console.print(f"[red]خطا در دیتابیس: {e}[/red]"); sys.exit(1)
    ...
    table = Table(title="منوی اصلی", box=box.ROUNDED)
    table.add_column("گزینه", style="cyan", justify="right")
    table.add_column("نام بخش", style="white")
    table.add_column("توضیحات", style="green")
    
    table.add_row("0", "خروج", "بستن برنامه")
    table.add_row("1", "همه اینباندها", "اعمال روی کل سرور")
    ...
    for idx, row in enumerate(inbounds):
        ...
        table.add_row(str(menu_idx), row['remark'], f"{client_count} کاربر")
    ...
    if main_choice == 0:
        console.print("[yellow]خروج.[/yellow]")
        sys.exit(0)
    ...
    if main_choice > 1:
        ...
        console.print(f"\n[bold green]مدیریت اینباند انتخاب شده (ID: {target_inbound_id})[/bold green]")
        console.print("[0] بازگشت")
        console.print("[1] اعمال روی [bold]همه کاربران[/bold] این اینباند")
        console.print("[2] انتخاب/جستجوی [bold]یک کاربر خاص[/bold]")
    ...
    if target_email:
        ...
        if final_client:
            ...
            console.print("[red]خطا: کاربر گم شد![/red]"); sys.exit(1)

    else:
        console.print(Panel("تنظیمات گروهی (Bulk Actions)", style="cyan"))
        console.print("\n[bold]🕒 سناریوی زمان:[/bold]")
        console.print("[0] بدون تغییر")
        console.print("[1] افزایش برای [bold]همه[/bold]")
        console.print("[2] فقط [bold]تمدید فعال‌ها[/bold] (بدون تغییر منقضی‌ها)")
        console.print("[3] فقط [bold]احیای منقضی‌ها[/bold] (تاریخ شروع از الان)")
        ...
        console.print("\n[bold]💾 سناریوی حجم:[/bold]")
        console.print("[0] بدون تغییر")
        console.print("[1] افزایش برای [bold]همه[/bold]")
        console.print("[2] فقط کسانی که [bold]حجمشان تمام شده[/bold]")
        console.print("[3] فقط کسانی که [bold]هنوز حجم دارند[/bold] (هدیه)")
        traffic_scenario = IntPrompt.ask("انتخاب", choices=["0", "1", "2", "3"], default=0)
        
        gb_to_add = 0
        if traffic_scenario != 0:
            gb_to_add = FloatPrompt.ask("مقدار گیگابایت", default=0.0)


    ms_to_add = days_to_add * 86400000 if 'days_to_add' in locals() else 0
    bytes_to_add = int(gb_to_add * 1073741824) if 'gb_to_add' in locals() else 0
    current_time = int(time.time() * 1000)
    ...
    with Progress(SpinnerColumn(), BarColumn(), TextColumn("[progress.description]{task.description}")) as progress:
        task = progress.add_task("در حال اعمال تغییرات...", total=len(inbounds))
        
        for row in inbounds:
            i_id = row['id']
            if target_inbound_id != 0 and i_id != target_inbound_id:
            ...
            try:
                ...
                for client in clients:
                    ...
                    if target_email and bulk_updates:
                        if 'expiryTime' in bulk_updates:
                            client['expiryTime'] = bulk_updates['expiryTime']
                            modified = True
                        if 'totalGB' in bulk_updates:
                            client['totalGB'] = bulk_updates['totalGB']
                            client['total'] = bulk_updates['totalGB']
                            modified = True
                        if 'RESET_USAGE' in bulk_updates:
                            reset_usage_list.append(email)
                        ...
                        if time_scenario == 1: # All
                            base = current_time if is_expired else (expiry if expiry > 0 else current_time)
                            if is_expired: base = current_time
                            else: base = expiry
                            
                            client['expiryTime'] = base + ms_to_add
                            modified = True

                        elif time_scenario == 2: # Only Active
                            if not is_expired and expiry > 0:
                                client['expiryTime'] = expiry + ms_to_add
                                modified = True
                                
                        elif time_scenario == 3: # Only Expired
                            if is_expired:
                                client['expiryTime'] = current_time + ms_to_add
                                modified = True
                        
                        if traffic_scenario == 1: # All
                            if total > 0:
                                client['totalGB'] = total + bytes_to_add
                                client['total'] = client['totalGB']
                                modified = True
                        elif traffic_scenario == 2: # Only Depleted
                            if is_depleted:
                                client['totalGB'] = total + bytes_to_add
                                client['total'] = client['totalGB']
                                modified = True
                        elif traffic_scenario == 3: # Only Active Volume
                            if not is_depleted and total > 0:
                                client['totalGB'] = total + bytes_to_add
                                client['total'] = client['totalGB']
                                modified = True
                    ...
                    if modified or email in reset_usage_list:
                        sql_updates.append((
                            client['expiryTime'],
                            new_tot,
                            1 if client.get('enable') else 0,
                            email
                        ))
                        stats['processed'] += 1

                if modified:
                    inbound_json_updates[i_id] = json.dumps(settings)
                    
            except Exception: pass
            progress.advance(task)

    if stats['processed'] == 0 and not reset_usage_list:
        console.print("[yellow]تغییری برای اعمال وجود ندارد.[/yellow]")
        if Confirm.ask("بازگشت به منوی اصلی؟"): main(); return
        sys.exit(0)

    console.print(Panel(
        f"کاربران آپدیت شده : [bold green]{stats['processed']}[/bold green]\n"
        f"کاربران فعال شده : [bold yellow]{stats['enabled']}[/bold yellow]\n"
        f"کاربران ریست مصرف : [bold cyan]{len(reset_usage_list)}[/bold cyan]",
        title="تایید نهایی"
    ))
    
    if not Confirm.ask("آیا تغییرات در دیتابیس ذخیره شود؟"):
        console.print("[red]لغو شد.[/red]")
        sys.exit(0)

    try:
        for iid, js in inbound_json_updates.items():
            cursor.execute("UPDATE inbounds SET settings = ? WHERE id = ?", (js, iid))
        ...
        console.print(f"[bold red]❌ خطای دیتابیس: {e}[/bold red]")
        conn.rollback()
    finally:
        conn.close()
